	...
	if(isset($_SESSION['x_admin']->id) AND $_SESSION['x_admin']->id){
		...
		function mapa_admin(){
			...
			if(isset($item->maps_lat) AND $item->maps_lat AND isset($item->maps_lng) AND $item->maps_lng){
				$return .= '<iframe src="'.DIR.'/plugins/Google/Maps/maps.php?lat='.$item->maps_lat.'&lng='.$item->maps_lng.'&zoom='.$item->maps_zoom.'&nome='.$item->nome.'" width="100%" height="380" background="no" scrolling="No" marginwidth="0" marginheight="0" frameborder="0"></iframe> ';
			} else {
		...
		if(isset($_POST['gravar']) AND $_POST['gravar']){
			...
			$mysql->campo['maps_lng'] = $dados['lng'];
			...
			$lng 	  = isset($value->maps_lng) ? $value->maps_lng : '';
			...
			$arr['html'] = '<div class="w700 itens mb10">
								<div class="p20 pt15">
									<form id="GoogleMaps" method="post" action="'.$_SERVER['SCRIPT_NAME'].'">
										...
										<label> <input type="radio" name="tipo" value="0" '.iff(!$tipo, 'checked').' onclick="show('.A.'.endereco'.A.');hide('.A.'.lat_lng'.A.');"> Localização </label> &nbsp &nbsp
										<label> <input type="radio" name="tipo" value="1" '.iff($tipo, 'checked').'  onclick="show('.A.'.lat_lng'.A.');hide('.A.'.endereco'.A.');"> Latitude e lnggitude </label>
										...
										<div class="lat_lng '.iff(!$tipo, 'dn').'">
											...
											<div class="wr12">
												<b>lnggitude:</b>
												<input type="text" name="lng" class="w200 design" value="'.$lng.'" >
											</div>
											<div class="clear h05"></div>
										</div>
